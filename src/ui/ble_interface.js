/**
 * BLE Interface v5.1.4-SCAN-FIX
 * FIX: Autoscan al iniciar y al volver a foreground
 * FIX: CSS keyframes animación scan
 * FIX: Manejo de errores en toggleScan
 ...
 * Base: v5.1.3-DEDUP-NO-NEXO-MAIN
   */
   ...
   console.log('[BLEInterface] GALA v5.1.4-SCAN-FIX iniciado');
   }
   _detectMeshType() {
   ...
   /* FIX v5.1.4: Autoscan al iniciar */
   var self = this;
   setTimeout(function() {
   self._autoStartScan();
   }, 2000);
   ...
   /* FIX v5.1.4: Reanudar scan al volver a primer plano */
   if (!self.isScanning) {
   self._autoStartScan();
   }
   }
   ...
   _autoStartScan() {
   var self = this;
   if (self.isDummyMode || !self.nativePlugin) return;
   ...
   self.updateStatus('Escaneando...');
   /* Auto-stop después de 15 segundos */
   setTimeout(function() {
   if (self.isScanning) {
   self._stopScanInternal();
   }
   }, 15000);
   })
   .catch(function(e) {
   ...
   /* FIX v5.1.4: Stop scan interno */
   _stopScanInternal() {
   var self = this;
   if (!self.isScanning) return;
   if (_hasNativeMethod(self.nativePlugin, 'stopScan')) {
   _safeNativeCall(self.nativePlugin, 'stopScan', {})
   .then(function() {
   self.isScanning = false;
   self.updateScanButton();
   self.updateStatus();
   ...
   /* FIX v5.1.4: CSS animación scan inyectada */
   var scanStyle = document.createElement('style');
   scanStyle.id = 'ble-scan-anim-style';
   scanStyle.textContent = '#ble-scan-btn{position:relative;}@keyframes scan-pulse{0%{box-shadow:0 0 0 0 rgba(0,200,255,0.7);}70%{box-shadow:0 0 0 15px rgba(0,200,255,0);}100%{box-shadow:0 0 0 0 rgba(0,200,255,0);}}#ble-scan-btn.scanning{animation:scan-pulse 1.5s infinite;background:linear-gradient(135deg,#00c8ff,#a855f7)!important;}#ble-scan-btn.scanning::after{content:"";position:absolute;top:-4px;left:-4px;right:-4px;bottom:-4px;border-radius:50%;border:2px solid rgba(0,200,255,0.5);animation:scan-pulse 1.5s infinite;}';
   ...
   if (self.isScanning) {
   if (_hasNativeMethod(self.nativePlugin, 'stopScan')) {
   return _safeNativeCall(self.nativePlugin, 'stopScan', {}).then(function() { self.isScanning = false; self.updateScanButton(); self.updateStatus(); }).catch(function(e) { self.isScanning = false; self.updateScanButton(); });
   }
   ...
   if (_hasNativeMethod(self.nativePlugin, 'startScan')) {
   return _safeNativeCall(self.nativePlugin, 'startScan', {}).then(function() { self.isScanning = true; self.updateScanButton(); }).catch(function(e) { self.isScanning = false; self.updateScanButton(); console.warn('[BLEInterface] startScan error:', e.message); });
   }
   ...
   /*
 * Focos de Interés:
 *    * Implementación de auto-scan al iniciar el componente.
 *    * Manejo de reanudación de escaneo al volver al primer plano (foreground).
 *    * Optimización de animaciones CSS para el estado de escaneo (keyframes).
 *    * Robustez en el método de stop de escaneo.
 *    * Firma del código: v5.1.4-SCAN-FIX
     */
