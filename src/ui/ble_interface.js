 ...
 * FIX: Métodos dentro de clase, duplicado isFullyReady eliminado, ACK compatibilidad
 * FIX v3.3: sendChatMessage y _syncContactMaps movidos dentro de BLEInterface
   */
...
// === FUNCIONES UUID/MAC ===
const BLE_UUID_STORAGE_KEY = 'nexo_device_uuid';
...
var m = _normId(mac);
return m && m !== 'null' && m !== 'undefined' && m.length >= 12 && /^[0-9a-f:]+$/.test(m);
}
function _isValidUUID(uuid) {
var u = _normId(uuid);
return u && u !== 'null' && u !== 'undefined' && u.length === 36 && /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(u);
}
// === FIN FUNCIONES UUID/MAC ===
const BLE_STATES = {
...
this._activeChatUUID = null;
this._localDeviceUUID = _getDeviceUUID();
// ACK callbacks
this._onMessageReceived = null;
this._onMessageAcked = null;
this._onMessageRead = null;
...
_getDeviceStateByUUID(uuid) {
var mac = this._uuidToMacMap.get(uuid);
if (mac) {
return this._getDeviceState(mac);
}
return this._getDeviceState(uuid);
}
...
var uuid = this._macToUuidMap.get(deviceId);
if (!uuid && _isValidMAC(deviceId)) {
uuid = _generateUUID();
this._macToUuidMap.set(deviceId, uuid);
this._uuidToMacMap.set(uuid, deviceId);
}
var uiDeviceId = uuid || deviceId;
var messageId = null;
...
if (messageId && this.nativePlugin && this.nativePlugin.sendAck) {
try {
this.nativePlugin.sendAck({ deviceId: deviceId, messageId: messageId });
} catch (e) {}
}
window.dispatchEvent(new CustomEvent('nexo:ble:messageReceived', {
detail: { deviceId: uiDeviceId, content, senderName, messageId, source: data.source || 'unknown', timestamp: data.timestamp || Date.now() }
}));
if (this._onMessageReceived) {
this._onMessageReceived({ mac: uiDeviceId, body: content, messageId: messageId, ts: data.timestamp || Date.now() });
}
if (this._activeChatDeviceId !== uiDeviceId) {
...
panel.id = 'ble-panel';
panel.innerHTML = <div class="ble-header"><h3>🔷 BLE Mesh</h3><button id="ble-close">✕</button></div> <div class="ble-tabs"> <button class="ble-tab-btn active" data-tab="discovery">Descubrir</button> <button class="ble-tab-btn" data-tab="added">Agregados</button> <button class="ble-tab-btn" data-tab="connected">Conectados</button> </div> <div class="ble-main-controls"> <button id="ble-visibility-btn" class="ble-btn-visibility btn-visibility-off" ${this.isDummyMode ? 'disabled' : ''}> <span class="btn-icon">🚫</span><span>Visibilidad desactivada</span> </button> <button id="ble-scan-btn" class="ble-btn-discover" ${this.isDummyMode ? 'disabled' : ''}> <span class="btn-icon">🔍</span><span id="text-discover">Descubrir</span> </button> </div> <div class="ble-secondary-controls"> <span id="ble-status" class="ble-status-offline">OFFLINE</span> </div> <div id="tab-discovery" class="ble-tab-content active"> <div class="ble-list" id="ble-devices-list"><p class="ble-empty">Presiona buscar para encontrar dispositivos</p></div> </div> <div id="tab-added" class="ble-tab-content"> <div class="ble-list" id="ble-added-list"><p class="ble-empty">No hay contactos agregados</p></div> </div> <div id="tab-connected" class="ble-tab-content"> <div class="ble-list" id="ble-connected-list"><p class="ble-empty">No hay dispositivos conectados</p></div> </div>;
...
var id = (device.id || device.address || '').toString().toLowerCase().trim();
if (!id || id === 'null' || id === 'undefined') return;
if (this.localDeviceAddress && id === this.localDeviceAddress) return;
var mac = device.address || device.id;
if (mac && _isValidMAC(mac)) {
var existingUUID = this._macToUuidMap.get(mac);
if (!existingUUID) {
var newUUID = _generateUUID();
this._macToUuidMap.set(mac, newUUID);
this._uuidToMacMap.set(newUUID, mac);
device.id = newUUID;
} else {
device.id = existingUUID;
}
id = device.id;
}
...
var device = this.foundDevices.get(deviceId) || this.connectedDevices.get(deviceId);
...
this._syncContactMaps();
var contact = _getBLEContacts().find(c => (c.id || c.address) === deviceId);
var mac = null;
var uuid = null;
if (contact) {
mac = contact.address || contact.id;
uuid = contact.id || contact.address;
} else if (device) {
mac = device.address || device.id;
uuid = device.id || device.address;
}
if (mac && _isValidMAC(mac)) {
this._activeChatMAC = mac;
localStorage.setItem('nexo_active_chat_mac', mac);
}
if (uuid && _isValidUUID(uuid)) {
this._activeChatUUID = uuid;
if (mac) {
this._uuidToMacMap.set(uuid, mac);
this._macToUuidMap.set(mac, uuid);
}
...
var state = this._getDeviceStateByUUID(deviceId);
...
list.innerHTML = '<p class="ble-empty">Presiona buscar para encontrar dispositivosercanos</p>';
return;
}
list.innerHTML = '';
...
item.innerHTML = <div class="ble-device-info"> <span class="ble-device-name">${device.name || 'NEXO Device'}</span> <span class="ble-device-id">${this._formatId(id)}</span> <span class="ble-device-rssi">📶 ${device.rssi || '?'} dBm</span> </div> <div class="ble-device-actions">${actionHtml}</div>;
...
item.innerHTML = <div class="ble-device-info"> <span class="ble-device-name">${contact.name || 'NEXO Device'}</span> <span class="ble-device-id">${this._formatId(id)}</span> </div> <div class="ble-device-actions"> <button class="ble-btn-write" onclick="bleInterface.openChat('${id}')">✉️ Escribir</button> <button class="ble-btn-disconnect" onclick="bleInterface.removeContact('${id}')">Eliminar</button> </div>;
list.appendChild(item);
});
}
renderConnectedList() {
...
const item = document.createElement('div');
item.className = 'ble-device-item';
item.innerHTML = <div class="ble-device-info"> <span class="ble-device-name">${device.name || 'NEXO Peer'}</span> <span class="ble-device-id">${this._formatId(id)}</span> <span class="ble-device-rssi" style="color: #00ff00;">● ${device.direction || 'Conectado'} ${stateLabel}</span> </div> <div class="ble-device-actions"> <button class="ble-btn-write" onclick="bleInterface.openChat('${id}')" ${!isReady ? 'disabled title="Esperando canal..."' : ''}>✉️ Escribir</button> <button class="ble-btn-disconnect" onclick="bleInterface.disconnect('${id}')">Desconectar</button> </div>;
list.appendChild(item);
});
}
...
// === MÉTODOS ACK (compatibilidad v5.1.2) ===
onMessageReceived(callback) { this._onMessageReceived = callback; }
onMessageAcked(callback) { this._onMessageAcked = callback; }
onMessageRead(callback) { this._onMessageRead = callback; }
onConnectionChanged(callback) { this._onConnectionChanged = callback; }
sendReadReceipt(mac, messageIds) {
if (!this.nativePlugin || !this.nativePlugin.sendReadReceipt) return Promise.resolve();
var macClean = mac;
if (this._uuidToMacMap.has(mac)) macClean = this._uuidToMacMap.get(mac);
return this.nativePlugin.sendReadReceipt({ deviceId: macClean, messageIds: messageIds });
}
// === sendChatMessage (DENTRO de la clase) ===
async sendChatMessage(message) {
if (!message || !message.trim()) {
this.showToast('⚠️ Mensaje vacío', 'warning');
return { success: false, error: 'EMPTY_MESSAGE' };
}
var deviceId = this._activeChatDeviceId;
if (!deviceId) {
this.showToast('⚠️ No hay chat activo', 'warning');
return { success: false, error: 'NO_ACTIVE_CHAT' };
}
var mac = this._uuidToMacMap.get(deviceId);
if (!mac) {
mac = this._activeChatMAC || localStorage.getItem('nexo_active_chat_mac');
}
if (!mac) {
this.showToast('⚠️ BLE directo falló: MAC de dispositivo no encontrada', 'warning');
return { success: false, error: 'MAC_NOT_FOUND', queued: true };
}
var content = message.trim();
var messageId = _generateUUID();
var senderName = window.currentUser?.name || 'NEXO User';
var payload = JSON.stringify({
messageId: messageId,
senderName: senderName,
content: content,
timestamp: Date.now(),
senderUUID: this._localDeviceUUID
});
try {
if (this.nativePlugin && this.nativePlugin.sendMessage) {
await this.nativePlugin.sendMessage({ deviceId: mac, message: payload });
console.log('[BLEInterface] Mensaje enviado vía BLE a', mac);
return { success: true, messageId: messageId, transport: 'ble' };
} else {
throw new Error('Plugin no disponible');
}
} catch (e) {
console.warn('[BLEInterface] sendMessage falló:', e.message);
this.showToast('⚠️ BLE directo falló: ' + e.message, 'warning');
return { success: false, error: e.message, queued: true };
}
}
// === _syncContactMaps (DENTRO de la clase) ===
_syncContactMaps() {
...
requestPermissions() {
if (window.BLEPermissions && window.BLEPermissions.ensure) {
return window.BLEPermissions.ensure();
}
return Promise.resolve(false);
}
getConnectedDevices() {
if (this.isDummyMode) return Promise.resolve({ devices: [] });
if (this.nativePlugin && this.nativePlugin.getConnectedDevices) {
return this.nativePlugin.getConnectedDevices();
}
