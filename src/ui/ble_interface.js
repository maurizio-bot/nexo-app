 ...
 * 3) FIX openChat: cierra panel BLE antes de abrir chat para que el input sea visible
 * 4) Handshake de nombres: intercambio automatico de deviceName al conectar
 * 5) Fragmentacion/Reensamblaje: mensajes >160 chars se parten en chunks con UUID
 * 6) Badge BLE corregido: solo cuenta peers conectados, NO fragmentos ni eventos
 * 7) Mensajes largos se renderizan como UNA sola burbuja (no en partes)
 * 8) Nombre real del peer persistente via localStorage + handshake
 * 9) Name cache inteligente preservado
 * 10) _normId robusto preservado
 * 11) Health Monitor preservado
 */
...
export class BLEInterface {
  ...
  updateVisibilityButton() {
    ...
    if (!this.canAdvertise) {
      ...
      if (text) text.textContent = 'Bluetooth apagado';
  ...
  async toggleVisibility() {
    ...
    if (!this.canAdvertise) { this.showToast('Necesitas activar Bluetooth', 'warning'); return; }
    try {
  ...
  async openChat(deviceId) {
    ...
    // FIX v3.7.1: Cerrar panel BLE ANTES de abrir chat para que el input sea visible
