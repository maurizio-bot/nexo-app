 ...
 * FIX: Estado online por lastSeen (3min) + reconnect scan cada 8s
 */
...
export class BLEInterface {
  ...
  _continueInit() {
    ...
    setTimeout(function() {
      if (!self.isDummyMode && self.nativePlugin) {
        ...
        self._startReconnectLoop();
      }
    }, 500);
  }
  _startReconnectLoop() {
    var self = this;
    if (self._reconnectInterval) return;
    self._reconnectInterval = setInterval(function() {
      if (self.isDummyMode || !self.nativePlugin || self.isScanning) return;
      var contacts = _getBLEContacts();
      var hasOffline = contacts.some(function(c) {
        return !c.online || (Date.now() - (c.lastSeen || 0)) > 30000;
      });
      if (hasOffline) {
        console.log('[BLEInterface] Reconnect scan');
        self._autoScanForKnownContacts();
      }
    }, 8000);
  }
  ...
  renderOnlineStrip() {
    ...
    var onlineContacts = contacts.filter(function(c) {
      return (Date.now() - (c.lastSeen || 0)) < 180000;
    });
  ...
  renderContactsList() {
    ...
    contacts.forEach(function(contact, index) {
      var uuid = _normId(contact.deviceUUID);
      var isOnline = (Date.now() - (contact.lastSeen || 0)) < 180000;
      var initials = _getInitials(contact.name);
      var gradClass = _getGradientForUUID(uuid);
...
// Focos de Interes:
// - Mantener la integridad del codigo fuente (v5.1.7-NO-MAC-CLEAN-FIX-BACK).
// - Validar el comportamiento de cierre del panel al finalizar un chat.
// - Asegurar la persistencia y carga de contactos y NEXO ID desde el vault.
// - Verificar la correcta funcionalidad de la interfaz visual y listeners de eventos.
