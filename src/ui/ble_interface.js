...
// === ACK + JUMP + HEARTBEAT ===
this._seenMsgs = new Map(); // deduplicación: msgId -> ts
this._pendingACKs = new Map(); // msgId -> { resolve, reject, timer, retries, sentAt, deviceId }
this._heartbeatTimers = new Map(); // deviceId -> { intervalId, lastPong, missed }
...
// Path truncado a 4 hops para deduplicación
...
// Enviar ACK delivered automático para mensajes de chat
...
if (idx2 >= 0) { contacts2[idx2].online = true; contacts2[idx2].lastSeen = Date.now(); contacts2[idx2].deviceId = deviceId; if (text && !isControl) contacts2[idx2].lastMessage = text.substring(0, 50); _saveBLEContacts(contacts2); self.renderContactsList(); self.renderOnlineStrip(); }
}
}
...
if (idx3 >= 0) { contacts3[idx3].unreadCount = (contacts3[idx3].unreadCount || 0) + 1; contacts3[idx3].lastMessage = text.substring(0, 50); contacts3[idx3].lastSeen = Date.now(); _saveBLEContacts(contacts3); self.renderContactsList(); self.renderOnlineStrip(); }
}
self.newDevicesCount++; self.updateBadge();
var stableId = senderUUID || deviceId;
_safeDispatchEvent('nexo:ble:messageReceived', { deviceId: stableId, deviceUUID: senderUUID, content: text, senderName: senderName, messageId: messageId, source: source, timestamp: data.timestamp || Date.now() });
...
// Disparar scan por acción del usuario (abrir panel, etc)
triggerScanByAction() {
...
// Si ya es contacto, solo actualizar online y conectar
...
list.innerHTML = '';
var contacts = _getBLEContacts();
// Deduplicación SOLO por NEXO ID
var seenNexoIds = {};
var deduped = [];
...
// Focos de Interés:
// - Deduplicación de contactos por NEXO ID.
// - Manejo de estados de conexión (GATT/Heartbeat).
// - Persistencia de datos en FileSystem (NexoBLE plugin) y localStorage.
// - Lógica de reintento en mensajes (ACK/Queue).
