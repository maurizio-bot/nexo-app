 ...
 * FIX: Mensajes se encolan si canal no listo, se envian al conectar
   */
   ...
   // Verificar si el device scaneado ya tiene UUID real (de conexión previa)
   var scannedDevice = this.foundDevices.get(mac);
   if (scannedDevice && scannedDevice.deviceUUID && _isBLEContact(scannedDevice.deviceUUID)) {
   this._autoConnectGATT(mac, device); this.foundDevices.delete(mac); this._closePanelAndRefresh(); return;
   }
