/**
 * NEXO App v5.0.18-ACK-JUMP-HEARTBEAT-FIXED
 * Base: v5.0.17-ACK-JUMP-HEARTBEAT
...
import { initBLEInterface } from '../ui/ble_ui.js';
...
class NexoApp {
  constructor(config) {
    ...
    /* NUEVO: ACK timeout config */
    this._ackTimeoutBase = 5000;
    this._ackTimeoutPerHop = 2000;
    this._ackMaxRetries = 2;
    DEBUG.log('NEXO v5.0.18-ACK-JUMP-HEARTBEAT-FIXED iniciando...', 'info', 'APP_INIT');
  }
  ...
  async init() {
    ...
    try {
      ...
      DEBUG.success('NEXO v5.0.18-ACK-JUMP-HEARTBEAT-FIXED Ready', 'APP_READY');
    } catch (err) {
        ...
        try {
          ...
          if (content.charAt(0) === '{' || (detail.data && detail.data.charAt(0) === '{')) {
            try {
              ...
              if (json.v === 2) {
                msgV2 = json;
                messageId = json.msgId || json.messageId;
          ...
          if (detail.isControl || (messageId && content && (content.indexOf('"type":"ack"') !== -1 || content.indexOf('"type":"read_receipt"') !== -1))) {
            try {
              var ctrl = JSON.parse(detail.content || detail.data || content);
              if (ctrl.type === 'ack' || ctrl.type === 'read_receipt') {
                var ackMsgId = ctrl.msgId || ctrl.messageId;
                var ackType = ctrl.ackType || (ctrl.type === 'read_receipt' ? 'read' : 'delivered');
                self._handleACK(ackMsgId, ackType);
                return;
              }
            } catch (ackErr) {}
          }
          if (content.indexOf('"type":"heartbeat"') !== -1) {
            return;
          }
          self._handleMessage({
            content: content,
            ...
            messageId: messageId || detail.messageId,
            deviceUUID: senderUUID || detail.deviceUUID || detail.deviceId,
            _own: false,
            msgV2: msgV2
          }, 'ble_direct');
          if (senderUUID && messageId && self.activeContact && _normId(self.activeContact.id) === _normId(senderUUID)) {
            setTimeout(function() { self._sendReadReceipt(messageId, senderUUID); }, 200);
          }
        } catch (handlerErr) {
  ...
  async sendMessage(msg) {
    ...
    try {
      ...
      this._pendingMessages.set(messageId, { status: 'pending', timestamp: Date.now(), recipient: targetId, retries: 0, content: content });
      this._handleMessage({
        content: content,
        _own: true,
        timestamp: Date.now(),
      ...
      if (targetId && targetTransport === 'ble' && this.bleInterface && typeof this.bleInterface.sendChatMessage === 'function') {
        try {
          ...
          this._startACKTimeout(messageId, targetId, 0);
          return true;
        } catch (e) {
  ...
  _handleACK(messageId, ackType) {
    ...
    if (!pending) {
      var foundKey = null;
      this._pendingMessages.forEach(function(v, k) {
        if (!foundKey && (k === messageId || k.indexOf(messageId) >= 0 || messageId.indexOf(k) >= 0)) {
          foundKey = k;
        }
      });
      if (foundKey) pending = this._pendingMessages.get(foundKey);
    }
    if (!pending) {
  ...
  _startACKTimeout(messageId, recipientId, retryCount) {
    var self = this;
    if (!messageId || !recipientId) return;
    var timeoutMs = self._ackTimeoutBase + (self._ackTimeoutPerHop || 0);
    setTimeout(function() {
      var pending = self._pendingMessages.get(messageId);
      if (!pending) return;
      if (pending.status === 'delivered' || pending.status === 'read') return;
      if (retryCount < (self._ackMaxRetries || 2)) {
        DEBUG.log('ACK timeout, reintentando msg ' + messageId + ' intento ' + (retryCount + 1), 'warn', 'ACK_RETRY');
        self._retryMessage(messageId, recipientId, retryCount + 1);
      } else {
        DEBUG.log('ACK max retries alcanzado para ' + messageId, 'error', 'ACK_FAIL');
        self._updateMessageStatus(messageId, 'failed');
      }
    }, timeoutMs);
  }

  _retryMessage(messageId, recipientId, retryCount) {
    var self = this;
    if (!self.bleInterface || !self.bleInterface.sendChatMessage) return;
    var pending = self._pendingMessages.get(messageId);
    if (!pending) return;
    var content = pending.content || pending.text || '';
    if (!content) return;
    self.bleInterface.sendChatMessage(recipientId, content, messageId)
      .then(function() {
        self._startACKTimeout(messageId, recipientId, retryCount);
      })
      .catch(function(e) {
        if (retryCount >= (self._ackMaxRetries || 2)) {
          self._updateMessageStatus(messageId, 'failed');
  ...
  _sendACK(deviceUUID, messageId) {
    ...
    var payload = JSON.stringify({ v: 2, type: 'ack', msgId: messageId, ackType: 'delivered', from: this.vault && this.vault.getIdentity ? this.vault.getIdentity() : '', ts: Date.now() });
  ...
  _sendReadReceipt(messageId, recipientId) {
    if (!messageId || !recipientId) return;
    if (!this.bleInterface || !this.bleInterface.sendChatMessage) return;
    var payload = JSON.stringify({ v: 2, type: 'read_receipt', msgId: messageId, from: this.vault && this.vault.getIdentity ? this.vault.getIdentity() : '', ts: Date.now() });
