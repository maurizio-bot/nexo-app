    ...
    try {
      ...
      try {
        ...
        this.stream = new TheStream(container, {
          onItemTap: (item) => DEBUG.log(`Tap en: ${item.id?.substr(0,8) || 'unknown'}`),
          onItemSwipe: (item, action) => DEBUG.log(`Swipe ${action} en mensaje`),
          onLoadMore: async () => DEBUG.log('Cargando más mensajes...')
        });
    ...
    try {
      ...
      if (this.stream && this.stream.appendItems) {
        const streamItem = {
          ...
          author: { name: msg._sender?.substring(0,8) || 'Unknown' },
