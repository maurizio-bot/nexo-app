...
export class BLEInterface {
  ...
  _checkStaleConnections() {
    try {
      ...
      stale.forEach(function(id) { console.log('[HEALTH] Desconectando peer stale:', id); this.disconnect(id); }, this);
  ...
  _sendHandshake(deviceId) {
    var self = this;
    try {
      var id = _normId(deviceId);
      if (self._handshakeSent.has(id)) return Promise.resolve();
      var hs = JSON.stringify({ _t: 'hs', _n: self.localDeviceName });
      return self._safeNativeCall('sendMessage', { deviceId: id, message: hs }, 5000).then(function() {
  ...
  _processCompletePayload(deviceId, content, senderName, messageId, data) {
    ...
    var resolvedName = senderName
      || self._peerNames[nid]
      || _getContactName(nid)
      || (self.connectedDevices.get(nid) && self.connectedDevices.get(nid).name)
      || (self.foundDevices.get(nid) && self.foundDevices.get(nid).name)
      || 'NEXO Peer';
  ...
  _sendFragments(deviceId, chunks, fragId, total, index) {
    var self = this;
  ...
  _startReconnect(deviceId) {
    ...
    self._setDeviceState(deviceId, BLE_STATES.RECONNECTING, { message: 'Reconectando... (intento ' + (currentAttempts + 1) + '/5)' });
    var attemptReconnect = function() {
      ...
      self._safeNativeCall('forceReconnect', { deviceId: deviceId }, 8000).then(function() {
  ...
  _processQueueItems(queue, index) {
    var self = this;
  ...
  _sendMessageNative(deviceId, content) {
    ...
    return this._safeNativeCall('sendMessage', { deviceId: targetId, message: content }, 8000);
  }
  ...
  _continueToggleVisibility(permsReady) {
    var self = this;
    if (!permsReady) {
      ...
      if (self.nativePlugin && self.nativePlugin.getConnectedDevices) {
        return self._safeNativeCall('getConnectedDevices', {}, 3000).then(function(result) {
          ...
          devices.forEach(function(d) {
            ...
            self.connectedDevices.set(nid, Object.assign({}, d, { id: nid, address: nid }));
          });
          self.renderConnectedList();
        }).catch(function(err) {});
      }
  ...
  addContact(deviceId) {
    ...
    if (!device) { this.showToast('Dispositivo no encontrado', 'error'); return Promise.resolve(); }
  ...
  openChat(deviceId) {
    ...
    if (!device) { self.showToast('Contacto no disponible', 'error'); return Promise.resolve(); }
      ...
      openChatPromise = new Promise(function(resolve, reject) {
        ...
        var checkReady = function() {
          ...
          if (s.state === BLE_STATES.NOTIFICATIONS_READY || s.state === BLE_STATES.READY_TO_CHAT) {
            clearTimeout(timeout); self._openChatTimeouts.delete(nid); resolve();
      ...
      try {
        if (window.ensureBLEPermissions) {
          openChatPromise = openChatPromise.then(function() {
            ...
            permsReady = result;
            if (!permsReady) { self.showToast('Permisos BLE requeridos para conectar', 'warning', 5000); return Promise.reject(new Error('No perms')); }
            console.log('[BLEInterface] Conectando a', nid, '...');
            return self._safeNativeCall('connectToDevice', { deviceId: device.id || device.address || nid }, 10000);
  ...
  connect(deviceId) {
    ...
    try {
      if (self.nativePlugin) {
        return self._safeNativeCall('connectToDevice', { deviceId: deviceId }, 10000).catch(function(err) {
          self.showToast('Error al conectar', 'error');
        });
      }
    } catch (err) { self.showToast('Error al conectar', 'error'); }
    return Promise.resolve();
  }
  ...
  disconnect(deviceId) {
    ...
    try {
      ...
      if (self._activeChatDeviceId === nid) { self._activeChatDeviceId = null; self.updateBadge(); }
      var chatTimeout = self._openChatTimeouts.get(nid);
      if (chatTimeout) { clearTimeout(chatTimeout); self._openChatTimeouts.delete(nid); }
      var device = self.connectedDevices.get(nid);
      var targetId = (device && device.id) || (device && device.address) || deviceId;
      if (self.nativePlugin) {
        return self._safeNativeCall('disconnectDevice', { deviceId: targetId }, 5000).catch(function(err) {});
      }
    } catch (err) {}
    return Promise.resolve();
  }
  ...
  updateStatus(customStatus) {
    ...
    try {
      if (self.nativePlugin && self.nativePlugin.isBluetoothEnabled) {
        ...
        }).catch(function(err) { self.elements.status.textContent = 'ERROR'; });
      }
    } catch (err) { self.elements.status.textContent = 'ERROR'; }
  }
