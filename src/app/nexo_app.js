/**
 * NEXO App v5.0.22-SEQFIX
 * Base: v5.0.21-VAULTFIX
 ...
 * SEQFIX: Preservar seq en pipeline de mensajes para ordenamiento robusto.
 */
...
class NexoApp {
  constructor(config) {
    ...
    DEBUG.log('NEXO v5.0.22-SEQFIX iniciando...', 'info', 'APP_INIT');
  }
  ...
  async init() {
    ...
    try {
      ...
      DEBUG.success('NEXO v5.0.22-SEQFIX Ready', 'APP_READY');
    } catch (err) {
        ...
        try {
          ...
          if (content.charAt(0) === '{' || (detail.data && detail.data.charAt(0) === '{')) {
            try {
              ...
              if (json.seq !== undefined) detail.seq = json.seq;
              ...
              if (json.payload && json.payload.seq !== undefined) detail.seq = json.payload.seq;
...
25. FIX v5.0.22-SEQFIX: Campo seq (uint8) en payload BLE para ordenamiento robusto.
*/
