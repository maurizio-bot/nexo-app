 ...
 * FIX: Mensajes se encolan si canal no listo, se envian al conectar
   */
   ...
   console.log('[BLE] onDeviceFound: mac=' + mac + ' name=' + (device.name || '') + ' rssi=' + (device.rssi || ''));
   if (!mac) return;
   if (this.localDeviceAddress && mac === this.localDeviceAddress) return;
   var knownUUID = this._macToUuidMap.get(mac);
   console.log('[BLE] onDeviceFound: knownUUID=' + (knownUUID || 'null') + ' isContact=' + (knownUUID ? _isBLEContact(knownUUID) : false));
   ...
   console.log('[BLE] onDeviceFound: NUEVO dispositivo. foundDevices.size=' + this.foundDevices.size);
   ...
   console.log('[BLE] _addNewDevice: mac=' + mac + ' name=' + name);
   var contacts = _getBLEContacts();
   console.log('[BLE] _addNewDevice: Total contactos=' + contacts.length);
   contacts.forEach(function(c, i) { console.log('[BLE] Contacto['+i+'] uuid=' + c.deviceUUID.substring(0,8) + '... mac=' + (c.macAddress || 'null') + ' name=' + c.name); });
   ...
   console.log('[BLE] _addNewDevice: CREANDO tempUUID=' + tempUUID);
...
// Focos de Interés:
// - Deduplicación de contactos por MAC.
// - Conexión GATT en background al abrir chat.
// - Manejo de estados de conexión (BLE_STATES).
// - Interacción nativa con plugin de Capacitor.
