 ...
 * FIX: openChat() ahora conecta via connectToDevice() y espera READY_TO_CHAT
 * FIX: sendChatMessage() publico, mapea UUID->MAC y enriquece payload JSON
 */
...
export class BLEInterface {
  ...
  // NUEVO v4.2.0: Metodo publico para enviar mensajes de chat
  async sendChatMessage(deviceUUID, content) {
    var uuid = _normId(deviceUUID);
    ...
    if (!mac) {
      ...
      mac = contact && contact.macAddress;
    }
    if (!mac) throw new Error('No se encontro MAC para el dispositivo ' + uuid);

    var state = this._getDeviceState(mac);
    if (state.state !== BLE_STATES.READY_TO_CHAT && state.state !== BLE_STATES.NOTIFICATIONS_READY) {
      throw new Error('Dispositivo no conectado. Estado: ' + state.state);
    }

    return this._sendMessageNative(mac, content);
  }
  ...
  renderContactsList() {
    ...
    contacts.forEach(function(contact) {
      ...
      // FIX v4.2.0: openChat ahora es async, conecta y espera READY_TO_CHAT
      chatBtn.addEventListener('click', async function() {
        chatBtn.disabled = true;
        chatBtn.textContent = '...';
        try {
          await self.openChat(uuid);
        } catch (e) {
          console.error('[BLEInterface] openChat error:', e);
        } finally {
  ...
  // FIX v4.2.0: openChat ahora es async. Conecta via connectToDevice() y espera READY_TO_CHAT.
  async openChat(deviceUUID) {
    ...
    // Conectar si no esta en estado READY_TO_CHAT
    var state = this._getDeviceState(mac);
    if (state.state !== BLE_STATES.READY_TO_CHAT && state.state !== BLE_STATES.NOTIFICATIONS_READY) {
      this.showToast('Conectando...', 'info', 5000);
      try {
        await this.nativePlugin.connectToDevice({ deviceId: mac });
        await this._waitForReadyToChat(mac, 15000);
      } catch (e) {
        this.showToast('No se pudo conectar: ' + e.message, 'error');
        this._activeChatDeviceId = null;
        this._activeChatMAC = null;
        return;
      }
