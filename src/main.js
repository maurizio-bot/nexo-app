...
//-----‐-----------------------------------------------------------------
/**
 * src/main.js - Punto de entrada NEXO v9.9.5-FASE4-FIXED-VAULT
...
try {
console.log('[MAIN] NEXO v9.9.5-FASE4-FIXED-VAULT iniciando...');
...
var isMac = deviceId && deviceId.indexOf(':') >= 0;
var contact = null;
var nxId = null;
if (!isMac) {
nxId = deviceId;
contact = vaultFindContactByNexoId(deviceId);
}
if (!contact && isMac && window.NEXO.app.bleInterface) {
var bleContact = window.NEXO.app.bleInterface.getContactByDeviceId(deviceId);
if (bleContact) {
nxId = bleContact.deviceUUID;
contact = { nexoId: nxId, displayName: bleContact.name || 'NEXO', deviceId: deviceId };
}
}
...
if (!contact) {
if (isMac) {
console.warn('[MAIN] Notificacion con MAC sin contacto conocido:', deviceId);
return;
}
contact = { nexoId: deviceId, displayName: 'NEXO', deviceId: null };
vaultSaveContact(contact);
}
if (!contact.name) contact.name = contact.displayName || 'NEXO';
if (!nxId) nxId = contact.nexoId;
window.NEXO.app.activeContact = contact;
if (window.NEXO.app.bleInterface) {
window.NEXO.app.bleInterface._activeChatDeviceId = nxId;
}
...
console.log('[MAIN] Chat abierto desde notificacion:', nxId);
} catch (e) {
console.warn('[MAIN] _openChatFromNotification error:', e);
}
}
//-----------------------------------------------------------------------------------------------------------------------
// PARTE 5 (UNICA - no duplicar)
//-----------------------------------------------------------------------------------------------------------------------
...
console.warn('[MAIN] vaultLoadMessages error:', e);
});
}
} catch (e) {
...
// FIX: Iniciar autoscan inmediatamente al arrancar
if (_autoScan && typeof _autoScan.start === 'function') {
_autoScan.start();
console.log('[MAIN] AutoScan iniciado');
}
