/**
 * BLE Interface v5.0.8-UI-FIX
 * FIXES: Botones superiores estilo EYE, colores unificados #0082FC, SCAN con borde arcoíris
 */
...
export class BLEInterface {
  constructor(bleMesh) {
    ...
    console.log('[BLEInterface] DUAL GATT v5.0.8-UI-FIX iniciado. MAC maps:', this._uuidToMacMap.size, 'entradas');
  }
  ...
  sendChatMessage(deviceUUID, content, messageId) {
    ...
    return new Promise(function(resolve, reject) {
      try {
        var uuid = _normId(deviceUUID);
        if (!uuid) {
          self.showToast('[BLE JS] sendChatMessage: ID vacio', 'error', 2000);
          reject(new Error('deviceUUID vacio'));
          return;
        }
        if (!content || typeof content !== 'string' || content.trim() === '') {
          self.showToast('[BLE JS] sendChatMessage: Mensaje vacio', 'warning', 2000);
  ...
  injectStyles() {
    ...
    style.textContent = "#ble-tab { position: fixed; left: 0; top: 50%; transform: translateY(-50%); width: 44px; height: 100px; background: #0082FC; border-radius: 0 12px 12px 0; display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer; z-index: 2147483644; color: #fff; font-weight: bold; } .ble-tab-badge { position: absolute; top: 5px; right: -5px; background: #ff4444; color: white; width: 18px; height: 18px; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-size: 10px; animation: pulse 2s infinite; } @keyframes pulse { 0%, 100% { transform: scale(1); } 50% { transform: scale(1.1); } } #ble-panel { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: #000000; transform: translateX(-100%); transition: transform 0.3s ease; z-index: 2147483645; color: #fff; display: flex; flex-direction: column; } #ble-panel.active { transform: translateX(0); } #ble-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); display: none; z-index: 2147483644; backdrop-filter: blur(4px); } #ble-overlay.active { display: block; } .ble-header { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; border-bottom: 1px solid #333; } .ble-header h3 { margin: 0; font-size: 18px; color: #fff; flex: 1; text-align: center; } /* FIX v5.0.8: ble-btn-back ahora es círculo azul igual que EYE */ .ble-btn-back { width: 44px; height: 44px; border-radius: 50%; border: 2px solid #333333; background: #0082FC; color: #fff; font-size: 20px; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; transition: all 0.3s; padding: 0; } .ble-btn-back::before { content: '\\2190'; font-size: 20px; color: #fff; } .ble-btn-back:hover { box-shadow: 0 0 12px rgba(0,130,252,0.4); } .ble-btn-visibility-round { width: 44px; height: 44px; border-radius: 50%; border: 2px solid #333333; background: rgba(255,255,255,0.1); color: #888; cursor: pointer; font-size: 12px; display: flex; align-items: center; justify-content: center; transition: all 0.3s; } .ble-btn-visibility-round.active { background: #0082FC; color: #fff; border-color: #0082FC; box-shadow: 0 0 12px rgba(0,130,252,0.4); } .ble-btn-visibility-round::before { content: 'EYE'; font-size: 10px; font-weight: bold; } .ble-status-bar { padding: 8px 20px; } .ble-status-offline { font-size: 12px; color: #888; } .ble-status-online { font-size: 12px; color: #0082FC; } .ble-status-scanning { font-size: 12px; color: #ffaa00; animation: blink 1s infinite; } @keyframes blink { 0%,50% { opacity: 1; } 51%,100% { opacity: 0.7; } } .ble-contacts-list { flex: 1; overflow-y: auto; padding: 0 20px; } .ble-contact-item { display: flex; align-items: center; justify-content: space-between; padding: 14px 16px; background: rgba(255,255,255,0.05); border: 1px solid #333; border-radius: 12px; margin-bottom: 10px; cursor: pointer; transition: all 0.2s; } .ble-contact-item:hover { background: rgba(0,130,252,0.1); border-color: #0082FC; } .ble-contact-item.online { border-left: 3px solid #0082FC; } .ble-contact-item.offline { border-left: 3px solid #666; } .ble-contact-info { display: flex; flex-direction: column; flex: 1; min-width: 0; } .ble-contact-name { font-weight: 600; font-size: 15px; color: #fff; } .ble-contact-status { font-size: 11px; color: #888; margin-top: 2px; } .ble-contact-actions { display: flex; gap: 8px; } /* FIX v5.0.8: ble-btn-chat azul #0082FC */ .ble-btn-chat { padding: 8px 16px; background: #0082FC; color: #fff; border: none; border-radius: 8px; cursor: pointer; font-size: 12px; font-weight: bold; transition: all 0.2s; } .ble-btn-chat:hover { box-shadow: 0 0 8px rgba(0,130,252,0.4); } .ble-btn-remove { padding: 8px 12px; background: #ff4444; color: #fff; border: none; border-radius: 8px; cursor: pointer; font-size: 12px; } .ble-empty { text-align: center; color: #666; padding: 40px 20px; font-style: italic; } .ble-bottom-bar { display: flex; align-items: center; justify-content: space-between; padding: 16px 20px; border-top: 1px solid #333; gap: 12px; } .ble-new-device { display: flex; align-items: center; gap: 10px; flex: 1; background: rgba(0,130,252,0.1); border: 1px solid #0082FC; border-radius: 12px; padding: 10px 14px; } .ble-new-device span { color: #fff; font-size: 14px; flex: 1; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; } .ble-btn-add-small { width: 36px; height: 36px; border-radius: 50%; background: #0082FC; color: #fff; border: none; font-size: 20px; font-weight: bold; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; } /* FIX v5.0.8: ble-btn-scan-round con borde arcoíris gradiente */ .ble-btn-scan-round { width: 56px; height: 56px; border-radius: 50%; background: #000; color: #fff; border: 3px solid transparent; background-clip: padding-box; position: relative; font-size: 10px; font-weight: bold; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0; box-shadow: 0 4px 15px rgba(0,130,252,0.3); transition: all 0.3s; } .ble-btn-scan-round::before { content: 'SCAN'; font-size: 10px; color: #fff; } .ble-btn-scan-round::after { content: ''; position: absolute; top: -3px; left: -3px; right: -3px; bottom: -3px; border-radius: 50%; background: conic-gradient(from 0deg, #00E5FF, #0082FC, #9D4EDD, #FF006E, #FF9E00, #FFD60A, #00E5FF); z-index: -1; } .ble-btn-scan-round.scanning { background: #ff4444; box-shadow: 0 0 20px rgba(255,68,68,0.4); animation: pulse-red 1.5s infinite; } .ble-btn-scan-round.scanning::before { content: 'STOP'; } .ble-btn-scan-round.scanning::after { background: conic-gradient(from 0deg, #ff4444, #cc0000, #ff4444); } @keyframes pulse-red { 0%,100% { box-shadow: 0 0 0 0 rgba(255,68,68,0.4); } 50% { box-shadow: 0 0 0 10px rgba(255,68,68,0); } } .ble-toast { position: fixed; bottom: 100px; left: 50%; transform: translateX(-50%); padding: 12px 24px; border-radius: 8px; color: #fff; font-weight: bold; z-index: 2147483646; animation: fadeInUp 0.3s ease; } .ble-toast.success { background: #0082FC; color: #fff; } .ble-toast.error { background: #ff4444; } .ble-toast.warning { background: #ffaa00; color: #000; } .ble-toast.info { background: #444; } @keyframes fadeInUp { from { opacity: 0; transform: translateX(-50%) translateY(20px); } to { opacity: 1; transform: translateX(-50%) translateY(0); } }";
  ...
  _autoConnectGATT(mac, device) {
    ...
    if (!self.nativePlugin || !_hasNativeMethod(self.nativePlugin, 'connectToDevice')) {
      ...
      return Promise.resolve();
        ...
        var macNorm = _normMac(mac);
    if (!macNorm) {
      self.showToast('[BLE JS] AutoConnect: MAC invalida', 'error', 2000);
      return Promise.resolve();
    }
    self.showToast('[BLE JS] AutoConnect: conectando a ' + macNorm, 'info', 2000);
    return _safeNativeCall(self.nativePlugin, 'connectToDevice', { deviceId: _macWithColons(macNorm) })
      .then(function(connResult) {
        if (connResult && (connResult.connected || connResult.alreadyConnected)) {
          self.showToast('[BLE JS] AutoConnect: conectado', 'success', 2000);
          self._setDeviceState(macNorm, BLE_STATES.CONNECTING);
          return self._waitForReadyToChat(macNorm, 10000);
        }
        throw new Error('No se pudo conectar');
      })
      .then(function() {
        self.showToast('[BLE JS] AutoConnect: READY_TO_CHAT', 'success', 2000);
      })
      .catch(function(e) {
        self.showToast('[BLE JS] AutoConnect fallo: ' + (e.message || 'Error'), 'warning', 3000);
        console.warn('[BLEInterface] AutoConnectGATT fallo:', e.message);
      });
  }
  ...
  removeContact(deviceUUID) {
    ...
    if (!uuid) return;
    var contact = _getContactByUUID(uuid);
    var name = contact ? contact.name : 'NEXO Peer';
    _removeBLEContact(uuid);
    this._uuidToMacMap.delete(uuid);
    this.renderContactsList();
    this.showToast('Eliminado: ' + name, 'info');
  }
  ...
  updateStatus(status) {
    ...
    if (status) {
      ...
      this.elements.status.className = status.indexOf('OFFLINE') !== -1 ? 'ble-status-offline' : 'ble-status-online';
      return;
    }
    if (this.isDummyMode) {
      this.elements.status.textContent = 'OFFLINE (Dummy)';
      this.elements.status.className = 'ble-status-offline';
      return;
    }
    var anyOnline = false;
    this.connectedDevices.forEach(function(d) {
      if (d && d.servicesReady) anyOnline = true;
    });
    if (anyOnline) {
