 ...
 * Integración con NexoBlePlugin.kt
   */
   ...
   data = fileData,
   currentState = ActiveTransfer.TransferState.PENDING,
   ...
   transfer.currentState = ActiveTransfer.TransferState.SENDING
   ...
   if (transfer == null || transfer.currentState == ActiveTransfer.TransferState.CANCELLED) {
   continue
   }
   ...
   currentState = ActiveTransfer.TransferState.RECEIVING
   )
   ...
   val msgId = json.optString("msgId", "")
   val transfer = activeTransfers[msgId] ?: return
   transfer.currentState = ActiveTransfer.TransferState.COMPLETED
   handler.post { callbacks?.onTransferComplete(msgId, true, null) }
   }
   // === HANDLE FILE CANCEL ===
   private fun handleFileCancel(deviceId: String, msgId: String) {
   val transfer = activeTransfers[msgId] ?: return
   transfer.currentState = ActiveTransfer.TransferState.CANCELLED
   ...
   val transfer = activeTransfers[msgId] ?: return
   transfer.currentState = ActiveTransfer.TransferState.PAUSED
   }
   fun resumeTransfer(msgId: String) {
   val transfer = activeTransfers[msgId] ?: return
   if (transfer.currentState == ActiveTransfer.TransferState.PAUSED) {
   transfer.currentState = ActiveTransfer.TransferState.SENDING
   processSendQueue()
   }
   }
   ...
   return activeTransfers.values.find { it.deviceId == deviceId && it.currentState == ActiveTransfer.TransferState.RECEIVING }
   }
