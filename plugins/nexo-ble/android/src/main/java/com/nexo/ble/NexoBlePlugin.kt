...
class NexoBlePlugin : Plugin() {
    ...
    companion object {
        private const val TAG = "NexoBlePlugin-ROBUST"
    ...
    fun connectToDevice(call: PluginCall) {
        try {
            ...
            /* FIX #11: Verificar si ya existe conexion activa antes de crear nueva */
            val existingGatt = gattClients[macNorm]
            if (existingGatt != null && clientConnectionStates[macNorm] == BluetoothProfile.STATE_CONNECTED) {
                remLog("INFO", "GATT_CLIENT", "Ya conectado a $macNorm, reutilizando")
                call.resolve(JSObject()
                    .put("connected", true)
                    .put("alreadyConnected", true)
                    .put("deviceId", rawDeviceId)
    ...
    fun startScan(call: PluginCall) {
        ...
        scanResults.clear()
        /* FIX #15: NO limpiar scannedDevices para preservar cache de BluetoothDevice */
        // scannedDevices.clear()
