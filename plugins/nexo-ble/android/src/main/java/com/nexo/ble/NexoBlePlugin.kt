...
// FIX: Siempre re-enviar intent al servicio para asegurar que reciba el NEXO ID actual
try {
...
val allGranted = if (Build.VERSION.SDK_INT >= Build.VERSION_SDK) {
...
// FIX: Notificar al servicio para reiniciar advertising con el nuevo NEXO ID
val ctx = activity.applicationContext
val intent = Intent(ctx, BleService::class.java)
...
// FIX: Scan sin filtro para detectar todos los dispositivos (filtrado por Manufacturer Data en callback)
...
/*
 1. Implementación de checkBLEStatus para verificar todos los estados de permisos BLE.
 2. Integración de initializeBLE con permissionsCallback para solicitud dinámica de permisos.
 3. Inclusión de lógica de persistencia de archivos (saveToFile, loadFromFile, deleteFile, listFiles).
 4. Re-implementación completa de la arquitectura GATT dual (Servidor + Cliente).
 5. Gestión de messageBuffers para reensamblaje de carga útil fragmentada.
 6. Gestión de reconnectTimers y keepAliveTimers para estabilidad de conexiones cliente.
   */
