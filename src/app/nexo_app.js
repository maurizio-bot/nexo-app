...
class NexoApp {
    ...
    try {
      ...
      this._nativeDeviceConnectedHandler = function(e) {
        try {
          ...
          /* FIX 2: Reintentar mensajes pendientes cuando contacto vuelve online */
          var peerUUID = detail.deviceUUID || (self.bleInterface && self.bleInterface._macToUuidMap ? self.bleInterface._macToUuidMap.get(detail.deviceId) : null);
          if (peerUUID) {
            setTimeout(function() { self._retryPendingMessages(peerUUID); }, 1500);
          }
        } catch (err) {
  ...
  /* ============================================================
     FIX 2: Reenvio automatico de mensajes pendientes
     ============================================================ */
  async _retryPendingMessages(contactId) {
    var cid = _normId(contactId);
    if (!cid) return;
    try {
      var stored = await this._loadMessagesFromVault(cid);
      var pending = stored.filter(function(m) {
        return m._own && (m.status === 'pending' || m.status === 'error');
      });
      if (pending.length === 0) return;
      DEBUG.log('Reintentando ' + pending.length + ' mensajes pendientes para ' + cid, 'info', 'RETRY');
      for (var i = 0; i < pending.length; i++) {
        var msg = pending[i];
        var success = await this.sendMessage({
          content: msg.content,
          recipient: cid,
          messageId: msg.messageId
        });
        if (success) {
          this._updateMessageStatusInVault(cid, msg.messageId, 'sent');
        }
      }
    } catch (e) {
      DEBUG.warn('Error reintentando mensajes: ' + e.message, 'RETRY_ERR');
    }
  }
  ...
  async sendMessage(msg) {
    ...
    try {
      ...
      /* === FIX 3: RENDER OPTIMISTA + GUARDAR EN VAULT === */
      var optimisticMsg = {
        content: content,
        _own: true,
        timestamp: Date.now(),
      ...
      this._handleMessage(optimisticMsg, 'self');
      this._saveMessageToVault(targetId, optimisticMsg);
  ...
  _handleMessage(msg, source) {
    ...
    try {
      ...
      /* FIX 3: Guardar mensaje en vault para persistencia con estado correcto */
      var vaultContactId = enriched._own ? enriched.recipient : (enriched.deviceUUID || enriched.sender);
      if (vaultContactId) {
        if (enriched._own) {
          this._updateMessageStatusInVault(vaultContactId, enriched.messageId, enriched.status);
        } else {
