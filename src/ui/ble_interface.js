 ...
 * FIX: Panel de contactos visible al arrancar (pantalla inicio = contactos)
   */
   ...
   panel.id = 'ble-panel';
   panel.innerHTML =
   '<div class="ble-header" style="position:relative;">' +
   '<button id="ble-back-btn" style="display:none;position:absolute;left:12px;top:50%;transform:translateY(-50%);width:36px;height:36px;border-radius:50%;background:rgba(255,255,255,0.1);border:none;cursor:pointer;align-items:center;justify-content:center;z-index:2;padding:0;">' +
   '<svg viewBox="0 0 24 24" width="20" height="20" fill="#fff" style="pointer-events:none;"><path d="M20 11H7.83l5.59-5.59L12 4l-8 8 8 8 1.41-1.41L7.83 13H20v-2z"/></svg>' +
   '</button>' +
   '<div class="contacts-title" id="ble-panel-title">Mensajes</div>' +
   '<div class="contacts-subtitle" id="ble-panel-subtitle">NEXO · Comunicacion P2P cifrada</div>' +
   '</div>' +
   ...
   this.elements.backBtn = document.getElementById('ble-back-btn');
   this.elements.panelTitle = document.getElementById('ble-panel-title');
   this.elements.panelSubtitle = document.getElementById('ble-panel-subtitle');
   ...
   this.elements.backBtn.addEventListener('click', function() { if (self.isScanning) self.toggleScan(); });
   }
   ...
   _enterScanUI() {
   if (this.elements.backBtn) this.elements.backBtn.style.display = 'flex';
   if (this.elements.panelTitle) this.elements.panelTitle.textContent = 'Escaneando...';
   if (this.elements.panelSubtitle) this.elements.panelSubtitle.style.display = 'none';
   }
   _exitScanUI() {
   if (this.elements.backBtn) this.elements.backBtn.style.display = 'none';
   if (this.elements.panelTitle) this.elements.panelTitle.textContent = 'Mensajes';
   if (this.elements.panelSubtitle) this.elements.panelSubtitle.style.display = 'block';
   }
   toggleScan() {
   var self = this;
   if (self.isDummyMode) return Promise.resolve();
   ...
   return _safeNativeCall(self.nativePlugin, 'stopScan', {}).then(function() { self.isScanning = false; self.updateScanButton(); self.updateStatus(); self._exitScanUI(); });
   }
   self.isScanning = false; self.updateScanButton(); self.updateStatus(); self._exitScanUI(); return Promise.resolve();
   } else {
   ...
   return _safeNativeCall(self.nativePlugin, 'startScan', {}).then(function() { self.isScanning = true; self.updateScanButton(); self._enterScanUI(); });
   }
   self.isScanning = true; self.updateScanButton(); self._enterScanUI(); return Promise.resolve();
   }
   ...
   self.isScanning = false; self.updateScanButton(); self.updateStatus(); self._exitScanUI();
   } else {
   self.foundDevices.clear(); self._renderedDeviceIds.clear(); self.renderContactsList(); self.renderNewDeviceBar(); self.renderOnlineStrip();
   if (_hasNativeMethod(self.nativePlugin, 'startScan')) {
   return _safeNativeCall(self.nativePlugin, 'startScan', {}).then(function() { self.isScanning = true; self.updateScanButton(); self._enterScanUI(); });
   }
   self.isScanning = true; self.updateScanButton(); self._enterScanUI();
   }
   return Promise.resolve();
   }
