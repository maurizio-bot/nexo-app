...
// 2) Contacto activo
else if (this.activeContact && this._normalizeId(this.activeContact.id) === convId) {
resolvedName = this.activeContact.name || resolvedName;
}
// 3) LocalStorage contacts
else if (!resolvedName || resolvedName === 'NEXO Peer' || resolvedName === 'Unknown') {
try {
...
console.log([BLE_SEND] Enviando via bleInterface a ${deviceId?.substring?.(0,8)}...);
try {
await this.bleInterface.sendMessage(deviceId, content);
DEBUG.success(📨 Enviado via BLE (fragmentado si >160chars) a ${deviceId?.substring?.(0,8)}, 'MSG_BLE');
return;
} catch (e) {
DEBUG.warn(bleInterface.sendMessage fallo, intentando fallback directo: ${e.message}, 'MSG_BLE_FALLBACK');
}
}
...
console.log([BLE_SEND] Fallback directo a ${deviceId?.substring?.(0,8)}...);
try {
await plugin.sendMessage({ deviceId, message: content });
DEBUG.success(📨 Enviado via BLE (directo) a ${deviceId?.substring?.(0,8)}, 'MSG_BLE');
} catch (e) {
