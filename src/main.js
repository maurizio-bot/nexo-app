 ...
 * Build #1605+ compatible. NO toca nativo.
   */
   ...
   if (!contactId) {
   *showAttachmentToast('No hay contacto seleccionado');
   return;
   }
   var attachmentData = {
   type: 'attachment',
   ...
   var photos;
   if (plugins.Camera.pickImages) {
   photos = await plugins.Camera.pickImages({ quality: 85, limit: 1 });
   if (photos && photos.photos && photos.photos.length > 0) {
   var photo = photos.photos[0];
   // pickImages devuelve Uri, necesitamos leer con Filesystem
   ...
   // Fallback a getPhoto con PHOTOS
   ...
   *fixLogoPath();
   ...
   contentDiv.innerHTML = '<div class="video-attachment" style="padding:8px 12px;background:rgba(0,0,0,0.3);border-radius:10px;">🎬 <b>Video</b>
   <span style="font-size:12px;opacity:0.7;">' + (attachment.meta.name || 'video.mp4') + '</span></div>';
